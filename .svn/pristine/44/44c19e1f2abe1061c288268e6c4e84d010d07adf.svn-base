...
import com.uyou.copenaccount.view.rv.animation.AlphaInAnimation;
import com.uyou.copenaccount.view.rv.animation.BaseAnimation;
import com.uyou.copenaccount.view.rv.entity.IExpandable;
import com.uyou.copenaccount.view.rv.loadmore.LoadMoreView;
import com.uyou.copenaccount.view.rv.loadmore.SimpleLoadMoreView;
import com.uyou.copenaccount.view.rv.refresh.ArrowRefreshHeader;
import com.uyou.copenaccount.view.rv.util.MultiTypeDelegate;
...
public abstract class BaseQuickAdapter<T, K extends BaseViewHolder> extends RecyclerView.Adapter<K> {
    ...
    public static final int REFRESH_VIEW = 0x00000666;
    ...
    /**
     * if need refresh or disableLoadMoreIfNotFullPage will first to call
     *
    ...
    /**
     * bind recyclerView {@link #setRecyclerView(RecyclerView)} before use!
     *
     ...
     */
    public void refresh() {
        if (isRefreshEnable() && mRequestRefreshListener != null && !isLoading()) {
     ...
     *
     * @param refreshHeader custom view
     */
    ...
    /**
     * set refresh enable and init something
     *
     * @param requestRefreshListener request to refresh
    ...
    public void setOnRefreshListener(RequestRefreshListener requestRefreshListener, RecyclerView recyclerView) {
        if (recyclerView instanceof PullRecyclerView) {
            ...
            if (recyclerView != null) {
                mRefreshHeader = new ArrowRefreshHeader(recyclerView.getContext());
    ...
    private void openRefresh(RequestRefreshListener requestRefreshListener) {
    ...
    public RequestRefreshListener getRequestRefreshListener() {
        return mRequestRefreshListener;
    }
    ...
    public int getLoadMoreViewPosition() {
        return (getRefreshViewCount()) + getHeaderLayoutCount() + mData.size() + getFooterLayoutCount();
    }
    ...
    /**
     * Returns the enable status for refresh
     *
     * @return True if refresh is enable, false otherwise
     */
    ...
    /**
     * setting up a new instance to data; default can load more
     *
     * @param data
     */
    ...
    public void addData(@IntRange(from = 0) int position, @NonNull T data) {
        ...
        int refreshPosition = getRefreshViewCount();
        notifyItemInserted(position + getHeaderLayoutCount() + refreshPosition);
    ...
    public void addData(@NonNull T data) {
        mData.add(data);
        int refreshPosition = getRefreshViewCount();
        notifyItemInserted(mData.size() + getHeaderLayoutCount() + refreshPosition);
        compatibilityDataSizeChanged(1);
    }
    ...
    public void remove(@IntRange(from = 0) int position) {
        ...
        int refreshPosition = getRefreshViewCount();
        int internalPosition = position + getHeaderLayoutCount() + refreshPosition;
    ...
    public void setData(@IntRange(from = 0) int index, @NonNull T data) {
        ...
        int refreshPosition = getRefreshViewCount();
        notifyItemChanged(index + getHeaderLayoutCount() + refreshPosition);
    }
    ...
    public void addData(@IntRange(from = 0) int position, @NonNull Collection<? extends T> newData) {
        ...
        int refreshPosition = getRefreshViewCount();
        notifyItemRangeInserted(position + getHeaderLayoutCount() + refreshPosition, newData.size());
    ...
    public void addData(@NonNull Collection<? extends T> newData) {
        if (newData != null) {
            ...
            int refreshPosition = getRefreshViewCount();
            notifyItemRangeInserted(mData.size() - newData.size() + getHeaderLayoutCount() + refreshPosition, newData.size());
            compatibilityDataSizeChanged(newData.size());
        }
    }
    ...
    public int getItemViewType(int position) {
        if (isRefreshEnable() && isRefreshHeader(position)) {
            return REFRESH_VIEW;
        }
        int refreshPosition = getRefreshViewCount();
        position = position - refreshPosition;  // 除去 refreshView 和 headViews
        if (getEmptyViewCount() == 1) {
    ...
    public K onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            case REFRESH_VIEW:
                baseViewHolder = getRefreshView(parent);
                break;
            case LOADING_VIEW:
    ...
    private K getRefreshView(ViewGroup parent) {
        if (mRefreshHeader == null) {
            mRefreshHeader = new ArrowRefreshHeader(parent.getContext());
            mRefreshHeader.setProgressStyle(mRefreshProgressStyle);
        }
        View view = mRefreshHeader;
        K holder = createBaseViewHolder(view);
        return holder;
    }
    ...
    public void onViewAttachedToWindow(K holder) {
        ...
        if (type == EMPTY_VIEW || type == HEADER_VIEW || type == FOOTER_VIEW || type == LOADING_VIEW || type == REFRESH_VIEW) {
            setFullSpan(holder);
        } else {
            addAnimation(holder);
        }
                ...
                public int getSpanSize(int position) {
                    ...
                    if (mSpanSizeLookup == null) {
                        ...
                        return (isFixedViewType(type)) ? gridManager.getSpanCount() : mSpanSizeLookup.getSpanSize(gridManager,
                                position - getHeaderLayoutCount() - refreshPosition);
                    }
                }
    ...
    protected boolean isFixedViewType(int type) {
        return type == EMPTY_VIEW || type == HEADER_VIEW || type == FOOTER_VIEW || type ==
                LOADING_VIEW || type == REFRESH_VIEW;
    }
    ...
    public void onBindViewHolder(K holder, int position) {
        ...
        switch (viewType) {
            case 0:
                convert(holder, getItem(position - getHeaderLayoutCount() - refreshPosition));
                break;
            case LOADING_VIEW:
    ...
    private void bindViewClickListener(final BaseViewHolder baseViewHolder) {
        ...
        final int refreshCount = getRefreshViewCount();
        if (getOnItemClickListener() != null) {
            view.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    getOnItemClickListener().onItemClick(BaseQuickAdapter.this, v, baseViewHolder.getLayoutPosition() - getHeaderLayoutCount() - refreshCount);
                }
            });
        }
        if (getOnItemLongClickListener() != null) {
            view.setOnLongClickListener(new View.OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    return getOnItemLongClickListener().onItemLongClick(BaseQuickAdapter.this, v, baseViewHolder.getLayoutPosition() - getHeaderLayoutCount() - refreshCount);
                }
            });
        }
    }
    ...
    private int getFooterViewPosition() {
        ...
        int refreshPosition = getRefreshViewCount();
        if (getEmptyViewCount() == 1) {
            int position = 1 + refreshPosition;
            if (mHeadAndEmptyEnable && getHeaderLayoutCount() != 0) {
                position++;
            }
            ...
            return getHeaderLayoutCount() + mData.size() + refreshPosition;
        }
        return -1;
    }
     ...
     * bind recyclerView {@link #setRecyclerView(RecyclerView)}} before use!
     *
     * @see #setRecyclerView(RecyclerView)
     */
...
//            }
        ...
        if (insert) {
            if (getEmptyViewCount() == 1) {
                int position = 0 + (getRefreshViewCount());
                if (mHeadAndEmptyEnable && getHeaderLayoutCount() != 0) {
                    position++;
                }
                notifyItemInserted(position);
    ...
    private void autoLoadMore(int position) {
        ...
        if (position < (getItemCount() - mPreLoadNumber - (getRefreshViewCount()))) {
            return;
        }
        if ((mLoadMoreView.getLoadMoreStatus() != LoadMoreView.STATUS_DEFAULT)
                || (isRefreshEnable() && mRefreshHeader.getState() == ArrowRefreshHeader.STATE_REFRESHING)) {
            return;
        }
...
//    }
    ...
    public int expand(@IntRange(from = 0) int position, boolean animate, boolean shouldNotify) {
        ...
        int parentPos = position + getHeaderLayoutCount() + refreshPosition;
        if (shouldNotify) {
            if (animate) {
    ...
    public int expandAll(int position, boolean animate, boolean notify) {
        ...
        int count = expand(position + getHeaderLayoutCount() + refreshPosition, false, false);
        for (int i = position + 1; i < this.mData.size(); i++) {
            ...
            if (isExpandable(item)) {
                count += expand(i + getHeaderLayoutCount() + refreshPosition, false, false);
            }
        }

        if (notify) {
            if (animate) {
                notifyItemRangeInserted(position + getHeaderLayoutCount() + refreshPosition + 1, count);
            } else {
                notifyDataSetChanged();
            }
        }
    ...
    public void expandAll() {
        int refreshPosition = getRefreshViewCount();
        for (int i = mData.size() - 1 + getHeaderLayoutCount() + refreshPosition; i >= getHeaderLayoutCount() + refreshPosition; i--) {
