package com.uyou.copenaccount.view.crop;
...
import com.uyou.copenaccount.view.crop.animation.SimpleValueAnimator;
import com.uyou.copenaccount.view.crop.animation.SimpleValueAnimatorListener;
import com.uyou.copenaccount.view.crop.animation.ValueAnimatorV14;
import com.uyou.copenaccount.view.crop.animation.ValueAnimatorV8;
import com.uyou.copenaccount.view.crop.callback.Callback;
import com.uyou.copenaccount.view.crop.callback.CropCallback;
import com.uyou.copenaccount.view.crop.callback.LoadCallback;
import com.uyou.copenaccount.view.crop.callback.SaveCallback;
import com.uyou.copenaccount.view.crop.util.Logger;
import com.uyou.copenaccount.view.crop.util.Utils;
...
public class CropImageView extends AppCompatImageView {
    ...
    private void handleStyleable(Context context, AttributeSet attrs, int defStyle, float mDensity) {
        TypedArray ta =
                context.obtainStyledAttributes(attrs, R.styleable.ucommon_scv_CropImageView, defStyle, 0);
        Drawable drawable;
        mCropMode = CropMode.SQUARE;
        try {
            drawable = ta.getDrawable(R.styleable.ucommon_scv_CropImageView_ucommon_scv_img_src);
            if (drawable != null) setImageDrawable(drawable);
            for (CropMode mode : CropMode.values()) {
                if (ta.getInt(R.styleable.ucommon_scv_CropImageView_ucommon_scv_crop_mode, 3) == mode.getId()) {
                    mCropMode = mode;
                    break;
                }
            }
            mBackgroundColor =
                    ta.getColor(R.styleable.ucommon_scv_CropImageView_ucommon_scv_background_color, TRANSPARENT);
            mOverlayColor =
                    ta.getColor(R.styleable.ucommon_scv_CropImageView_ucommon_scv_overlay_color, TRANSLUCENT_BLACK);
            mFrameColor = ta.getColor(R.styleable.ucommon_scv_CropImageView_ucommon_scv_frame_color, WHITE);
            mHandleColor = ta.getColor(R.styleable.ucommon_scv_CropImageView_ucommon_scv_handle_color, WHITE);
            mGuideColor = ta.getColor(R.styleable.ucommon_scv_CropImageView_ucommon_scv_guide_color, TRANSLUCENT_WHITE);
            for (ShowMode mode : ShowMode.values()) {
                if (ta.getInt(R.styleable.ucommon_scv_CropImageView_ucommon_scv_guide_show_mode, 1) == mode.getId()) {
                    mGuideShowMode = mode;
                    break;
                }
            }
            ...
            for (ShowMode mode : ShowMode.values()) {
                if (ta.getInt(R.styleable.ucommon_scv_CropImageView_ucommon_scv_handle_show_mode, 1) == mode.getId()) {
            ...
            mHandleSize = ta.getDimensionPixelSize(R.styleable.ucommon_scv_CropImageView_ucommon_scv_handle_size,
                    (int) (HANDLE_SIZE_IN_DP * mDensity));
            mTouchPadding = ta.getDimensionPixelSize(R.styleable.ucommon_scv_CropImageView_ucommon_scv_touch_padding, 0);
            mMinFrameSize = ta.getDimensionPixelSize(R.styleable.ucommon_scv_CropImageView_ucommon_scv_min_frame_size,
                    (int) (MIN_FRAME_SIZE_IN_DP * mDensity));
            mFrameStrokeWeight =
                    ta.getDimensionPixelSize(R.styleable.ucommon_scv_CropImageView_ucommon_scv_frame_stroke_weight,
                            (int) (FRAME_STROKE_WEIGHT_IN_DP * mDensity));
            mGuideStrokeWeight =
                    ta.getDimensionPixelSize(R.styleable.ucommon_scv_CropImageView_ucommon_scv_guide_stroke_weight,
                            (int) (GUIDE_STROKE_WEIGHT_IN_DP * mDensity));
            mIsCropEnabled = ta.getBoolean(R.styleable.ucommon_scv_CropImageView_ucommon_scv_crop_enabled, true);
            mInitialFrameScale = constrain(
                    ta.getFloat(R.styleable.ucommon_scv_CropImageView_ucommon_scv_initial_frame_scale,
                            DEFAULT_INITIAL_FRAME_SCALE), 0.01f, 1.0f, DEFAULT_INITIAL_FRAME_SCALE);
            mIsAnimationEnabled =
                    ta.getBoolean(R.styleable.ucommon_scv_CropImageView_ucommon_scv_animation_enabled, true);
            mAnimationDurationMillis = ta.getInt(R.styleable.ucommon_scv_CropImageView_ucommon_scv_animation_duration,
                    DEFAULT_ANIMATION_DURATION_MILLIS);
            mIsHandleShadowEnabled =
                    ta.getBoolean(R.styleable.ucommon_scv_CropImageView_ucommon_scv_handle_shadow_enabled, true);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            ta.recycle();
    ...
    private Rect calcCropRect(int originalImageWidth, int originalImageHeight) {
        ...
        top = Math.max(top, 0);
        right = Math.min(right, imageW);
        bottom = Math.min(bottom, imageH);
        ...
        if (needRatio != 0) {
            if (width > 0 && height > 0) {
                ...
                Logger.e("width : " + width + " , height : " + height + " , ratio : " + ratio);
                ...
                if (ratio < 1.776f) {
                    ...
                    while (ratio < 1.776) {
                        if (isWidthMax) {
                            if (left > 0) {
                                left--;
                            } else if (right < (imageW - 1)) {
                                right++;
                            }
                        } else {
                            if (top > 0) {
                                top--;
                            } else if (bottom < (imageH - 1)) {
                                bottom++;
                            }
                        }
                        width = right - left;
        ...
        Logger.e("left : " + left + " , right : " + right + " , top : " + top + " , bottom : " + bottom
                + " , width : " + (right - left) + " , height : " + (bottom - top)
                + " , radio : " + needRatio + " , cropRatio : " + ratio);
    ...
    private float getRatio(int height, int width) {
        float fals = 0;
        if (height >= width) {
            fals = (float) height / width;
        } else {
            fals = (float) width / height;
        }
        BigDecimal b = new BigDecimal(fals);
        // 四舍五入
