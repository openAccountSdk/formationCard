package com.uyou.copenaccount.view.rv.stickheader;
...
import com.uyou.copenaccount.view.rv.stickheader.caching.HeaderProvider;
import com.uyou.copenaccount.view.rv.stickheader.caching.HeaderViewCache;
import com.uyou.copenaccount.view.rv.stickheader.calculation.DimensionCalculator;
import com.uyou.copenaccount.view.rv.stickheader.rendering.HeaderRenderer;
import com.uyou.copenaccount.view.rv.stickheader.util.LinearLayoutOrientationProvider;
import com.uyou.copenaccount.view.rv.stickheader.util.OrientationProvider;


public class StickyRecyclerHeadersDecoration extends RecyclerView.ItemDecoration {

    protected final StickyRecyclerHeadersAdapter mAdapter;
    protected final ItemVisibilityAdapter mVisibilityAdapter;
    protected final SparseArray<Rect> mHeaderRects = new SparseArray<>();
    protected final HeaderProvider mHeaderProvider;
    protected final OrientationProvider mOrientationProvider;
    protected final HeaderPositionCalculator mHeaderPositionCalculator;
    protected final HeaderRenderer mRenderer;
    protected final DimensionCalculator mDimensionCalculator;
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (mAdapter instanceof BaseQuickAdapter) {
            BaseQuickAdapter adapter = (BaseQuickAdapter) mAdapter;
            int headerCount = adapter.getHeaderLayoutCount() + adapter.getRefreshViewCount();
            if (adapter.getData().size() <= 0) {
                headerCount += adapter.getEmptyViewCount();
            }
            if (itemPosition < headerCount) {
                // 过滤头部+下拉刷新+空
                return;
            }
            if (adapter.getLoadMoreViewCount() > 0 && (itemPosition - headerCount) > adapter.getData().size()) {
                // 过滤加载更多
                return;
            }
        }
    ...
    public void onDrawOver(Canvas canvas, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (mAdapter instanceof BaseQuickAdapter) {
                ...
                if (adapter.getLoadMoreViewCount() > 0 && (position - headerCount) > adapter.getData().size()) {
                    // 过滤加载更多
                    continue;
                }
            }
